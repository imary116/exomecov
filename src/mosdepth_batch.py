...
def depth(b, cram, bed, ref, label):
    ...
    j.command(f'''mosdepth -n -b {bed} {j.ofile} {cram.cram} -f {ref}''')
    return j
...
if __name__ == '__main__':
    ...
    # human reference fasta for decoding CRAM
    ref = b.read_input('gs://genomics-public-data/resources/broad/hg38/v0/Homo_sapiens_assembly38.fasta')
    ...
    for path in cram_file_paths:
        ...
        j = depth(b, cram, bed, ref, label)  # run depth function
    ...
    # see what happens, but comment out after when you work on the merge command - took an hour to run
        b.write_output(j.ofile, f'gs://imary116/data/mosdepth-coverage/{label}.tsv') #write the output file in a google cloud bucket
